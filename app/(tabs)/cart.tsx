...
export default function Cart() {
  ...
  <View style={tw`flex-1 bg-stone-50 relative`}>
    ...
    {totalCount > 0 && (
      <View style={tw`p-6 bg-stone-50/90 border-t border-stone-200`}>
        <TouchableOpacity
          onPress={() => router.replace(`/screens/checkout`)}
          activeOpacity={0.8}
          style={tw`bg-primary py-3.5 px-4 rounded-xl items-center justify-center shadow-sm`}
        >
          <Text
            style={[
              tw`text-xs uppercase tracking-widest text-white font-semibold`,
              { fontFamily: "monospace" },
            ]}
          >
            Proceed to Checkout
